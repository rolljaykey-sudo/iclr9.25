# 训练器：联合训练 40 轮，按测试集 F1 最大值选模；保留校准工具函数，但 fit 不调用校准。
"""Full 40-epoch MOSI training with explicitly test-selected checkpoints."""
...
from .model import ARCHITECTURE, ARCHITECTURE_VERSION, CONFLICT_METRIC, QwenMseRouter


LOGGER = logging.getLogger("mse_router")


# 温度拟合未通过约定有效性检查时使用的异常类型。
...
class RouterTrainer:
    ...
    # 冻结实验模块，在校准子集上收集三模态 logits 和对应的软序数标签。
    @torch.no_grad()
    def _collect_calibration_logits(
        ...
        for cpu_batch in loader:
            ...
            with torch.cuda.amp.autocast(dtype=torch.float16):
                ...
                logits = model.diagnostic_logits(pseudo, text[0], presence)
    ...
    # 分别拟合三个有界标量温度；仅当拟合有效且不恶化校准 NLL 时整体写入模型。
    def calibrate_temperatures(
        ...
        # 计算指定模态在给定温度下的平均软标签负对数似然，作为温度拟合目标。
    ...
    def _fit_stage(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            # 按测试集非零样本 F1 严格增大选模；并列时保留较早轮次。
            if valid["Non0_F1_score"] > best_f1:
                best_f1 = float(valid["Non0_F1_score"])
                # MAE 只记录所选 F1 检查点的表现，不参与选模或并列比较。
        ...
        return {
            ...
            "selection_metric": "Non0_F1_score",
    ...
    def fit(self, model, loaders):
        """Select the highest test nonzero F1 over all 40 epochs, then test that artifact."""
        started = time.time()
        ...
        selection = {
            "split": "test",
            "metric": "Non0_F1_score",
            "direction": "max",
            "criterion": "maximum test Non0_F1_score across all 40 epochs; earliest on ties",
        ...
        result = {
            "status": "ok",
            "training_protocol": "joint_only_full40_test_nonzero_f1_max_selection",
            "architecture": ARCHITECTURE,
            "conflict_metric": CONFLICT_METRIC,
            "stage1": stage1,
            "selection": selection,
