#!/usr/bin/env python3
# MOSI 骨干迁移入口：沿用基线实验配置，适配目标模型的隐藏维度与分词器。
"""Transfer the verified ChatGLM MOSI configuration to another frozen backbone."""
import argparse
import json
import sys
from pathlib import Path

import run_chatglm3_mosi as baseline

ROOT = Path(__file__).resolve().parents[1]
OUTPUT = ROOT.parents[1] / "outputs" / "mosi_backbone_lr1e3_d01_20260912"
MODELS = {
    ...
    "qwen25": Path("/gpfs/work/cpt/jiachenhou23/models/Qwen/Qwen2.5-1.5B"),
}
...
def configure(target):
    ...
    def build_config(args, microbatch, accumulation):
        ...
        c.feature_dims = (hidden, 5, 20)
        c.router_backbone = "llama2" if target in ("llama2", "llama32", "qwen25") else "qwen18"
        return c
    ...
    if target == "qwen18":
        ...
        # 为 Qwen2.5 覆盖骨干加载接口，其余路由计算沿用统一实现。
        class Qwen25Router(BackboneMseRouter):
    ...
    def validate_files(args, hash_dataset):
        ...
        if digest is not None and digest != h.EXPECTED_DATASET_SHA256:
            raise RuntimeError("Dataset SHA256 differs from verified MOSI baseline")
