#!/usr/bin/env python3
"""Fresh ChatGLM3 MOSEI V2 runs with all training-input augmentation disabled."""
...
OUTPUT_ROOT = ROUTER_ROOT / "outputs/chatglm3-mosei-router-v2-no-augmentation"
BASELINE_ROOT = ROUTER_ROOT / "outputs/chatglm3-mosei-router-v2"
SEEDS = (3333, 4444, 5555)
...
def parse_args() -> argparse.Namespace:
    ...
    # Restrict retries to a new directory within this experiment, never Qwen,
    # historical GLM, or another Router experiment.
    root = args.output_root.resolve()
    if not root.is_relative_to(OUTPUT_ROOT.resolve()):
        parser.error("output root must be inside the GLM no-augmentation experiment")
...
def build_config(args: argparse.Namespace, microbatch: int, accumulation: int) -> Any:
    ...
    # Exactly the original GLM V2 backbone-specific configuration.
...
def validate_files(args: argparse.Namespace, hash_dataset: bool) -> dict[str, Any]:
    ...
    return {
        ...
        "model": {
            "backbone": "chatglm3", "path": str(model),
            "files": {
...
def configure_harness() -> None:
    ...
    harness.ROUTER_SOURCE_FILES = (
        *(REVISION / "mse_router" / name for name in (
        ...
        REVISION / "scripts/chatglm3_noaug.slurm",
        ...
        *(UPSTREAM / name for name in (
            ...
            "models/ChatGLM3/configuration_chatglm.py",
            "models/ChatGLM3/modeling_chatglm.py",
            "models/ChatGLM3/tokenization_chatglm.py",
        )),
    )
    harness.parse_args = parse_args
    harness.build_config = build_config
    harness.validate_files = validate_files
    # This change is process-local. Running Qwen and original GLM jobs keep
    # their own model classes, source fingerprints, and output directories.
