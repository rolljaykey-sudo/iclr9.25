...
def make_calibration_split(
    ...
    # Preserve seven ordinal strata, mapped to the SIMS v2 [-1, 1] range.
...
def augmentation_config(mode: str) -> dict[str, Any]:
    if mode not in ("standard", "none"):
        raise ValueError(f"unknown training augmentation: {mode}")
    enabled = mode == "standard"
    return {
        "mode": mode,
        "modality_drop_probability": 0.30 if enabled else 0.0,
        "audio_noise_probability": 0.20 if enabled else 0.0,
        "vision_mask_probability": 0.20 if enabled else 0.0,
    }


def prepare_training_batch(
    batch: dict[str, Any], mode: str = "standard"
) -> tuple[dict[str, Any], torch.Tensor]:
    """Choose input augmentation without changing model-internal Dropout.

    The clean branch neither mutates inputs nor consumes RNG state. Padding
    masks and lengths remain the original dataset's masks and lengths.
    """
    augmentation_config(mode)
    if mode == "standard":
        return augment_modalities(batch)
    presence = torch.ones(
        batch["text"].shape[0], 3,
        device=batch["text"].device, dtype=torch.float32,
    )
