"""Llama2 and ChatGLM3 backbones for the original V2 router.
...
are inherited unchanged so backbone comparisons keep the V2 semantics.
"""
...
class BackboneMseRouter(QwenMseRouter):
    """Original V2 router with a frozen Llama2 or ChatGLM3 language model."""

    def __init__(self, args: Any) -> None:
        # QwenMseRouter.__init__ is deliberately not called because it loads a
        # Qwen checkpoint. All experiment modules below mirror its V2 layout.
        ...
        # Keep the V2 seven-output ordinal head and 30-input router; change units.
    ...
    def diagnostic_logits(self, pseudo: torch.Tensor) -> torch.Tensor:
        ...
        last_valid = self._backbone_last_hidden(
            sequence, attention_mask, position_ids
        )
