#!/usr/bin/env python3
"""Qwen-1.8B Router V2 on supervised CH-SIMS v2, without input augmentation."""
...
UPSTREAM = harness.ADAPTER_DIR / "MSE-Qwen-1.8B"
MODEL = ROUTER_ROOT.parent / "models/Qwen/Qwen-1_8B"
DATASET = ROUTER_ROOT.parent / "datasets/MSA/data/SIMS_V2/ch-simsv2s.pkl"
OUTPUT_ROOT = ROUTER_ROOT / "outputs/qwen-simsv2-router-v2-no-augmentation"
...
PROTOCOL = {
    "dataset": "simsv2", "backbone": "qwen-1.8b", "training_augmentation": "none",
...
def parse_args() -> argparse.Namespace:
    ...
    if not args.output_root.resolve().is_relative_to(OUTPUT_ROOT.resolve()):
        parser.error("output root must be within the Qwen SIMS v2 experiment")
...
def aggregate_results(root: Path, require_all: bool = True) -> dict | None:
    ...
    with (root / "aggregate.lock").open("a") as lock:
        ...
        lines = ["# Qwen-1.8B SIMS v2, no augmentation", "",
                 "Seeds: 1111, 2222, 3333. Clean test; sample standard deviation (ddof=1).", "",
...
def configure_harness(args: argparse.Namespace) -> None:
    ...
    harness.EXPECTED_SPLITS = {key: value["samples"] for key, value in identity["splits"].items()}
    harness.DEFAULT_OUTPUT_ROOT = OUTPUT_ROOT
    harness.ROUTER_SOURCE_FILES = (
        *(REVISION / "mse_router" / name for name in (
            "math_utils.py", "data.py", "model.py", "trainer.py")),
        REVISION / "scripts/run_qwen_mosei_router.py", REVISION / "scripts/qwen_simsv2.slurm",
    ...
    original_model = router_model.QwenMseRouter

    class Simsv2QwenMseRouter(original_model):
        def __init__(self, config: Any) -> None:
            super().__init__(config)
            with torch.no_grad():
                self.anchors.copy_(torch.linspace(-1.0, 1.0, 7, device=self.anchors.device))

    router_model.QwenMseRouter = Simsv2QwenMseRouter
    ...
    def train(parsed):
        ...
        if checked.get("dataset_protocol") != PROTOCOL:
            raise ValueError("SIMS v2 Qwen preflight protocol mismatch")
