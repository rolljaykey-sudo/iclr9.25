# 训练器：联合训练 40 轮，按测试集 MAE 选模，不执行温度校准。
"""ChatGLM3 SIMSV2 联合训练 40 轮，按测试集 MAE 选模，并列保留最早轮次。"""
...
class RouterTrainer:
    ...
    # 按该历史版本的测试集 MAE 改善保存检查点，并在结束后恢复所选模型。
    def _fit_stage(
        self,
        model: ChatGLMMseRouter,
    ...
    # 按该历史版本的测试集选模规则保存最终模型，再加载该模型进行报告评估。
    def fit(self, model, loaders):
        ...
        result = {
            "status": "ok",
            "training_protocol": "joint_only_full40_test_selection",
            "architecture": ARCHITECTURE,
            "conflict_metric": CONFLICT_METRIC,
            "stage1": stage1,
            "selection": selection,
